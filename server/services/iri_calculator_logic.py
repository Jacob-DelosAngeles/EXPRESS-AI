...
class IRICalculator:
    ...
    #Create Segments of specified length - MEMORY OPTIMIZED with SPEED-AWARE IRI
        ...
        for start_dist in np.arange(0, max_distance, segment_length):
            ...
            if end_idx >= start_idx:
                ...
                if mean_speed >= self.MIN_VALID_SPEED:
                    ...
                    iri_value = self.K * rms_accel / (mean_speed ** self.SPEED_EXPONENT)
                    speed_flag = 'normal'
                elif mean_speed > 0:
                    # Low-speed segment: extrapolate what IRI would be at reference speed
                    extrapolated_iri = self.K * rms_accel / (self.REFERENCE_SPEED ** self.SPEED_EXPONENT)
                    # Cap to prevent unrealistic values
                    iri_value = min(extrapolated_iri, self.MAX_IRI_CAP)
                    speed_flag = 'low_speed'
                else:
                    # Vehicle stopped - minimal reliable data
                    iri_value = 0.0
                    speed_flag = 'stopped'
                ...
                segment = {
                    ...
                    'speed_flag': speed_flag,  # 'normal', 'low_speed', or 'stopped'
                }
