...
const DesktopHome = () => {
    ...
    const fetchJobs = useCallback(async () => {
        try {
            const res = await fetch(`${BACKEND_URL}/api/v1/desktop/jobs`);
            if (res.ok) {
    ...
    useEffect(() => {
        fetchJobs();
        // Refresh job list every 5s so the table updates as pipelines complete
        const id = setInterval(fetchJobs, 5000);
        return () => clearInterval(id);
    }, [fetchJobs]);

    // ── Handlers ────────────────────────────────────────────────────────────
    const onJobStarted = (jobId, jobName) => {
    ...
    const navItems = [
        ...
        { id: 'upload',    label: 'Upload',    icon: Icon.upload,    path: null },
        ...
        <div className="dh-root">
            ...
            <aside className="dh-sidebar">
                    ...
                    {navItems.map((item) => (
                        <button
                            ...
                            onClick={() => {
                                ...
                                if (item.id === 'upload') setShowUpload(true);
                ...
                <div className="dh-sidebar-footer">
                    ...
                    <div className="dh-session-row">
                        ...
                        <span className="dh-version-badge">v2.1.1</span>
                    </div>
                </div>
            </aside>
            ...
            <main className="dh-main">
                <div className="dh-content">
                    ...
                    <section className="dh-hero-card">
                        ...
                        <div className="dh-hero-body">
                            ...
                            <div className="dh-hero-actions">
                                <button className="dh-btn-primary" onClick={() => setShowUpload(true)}>
                                    {Icon.upload}
                                    Upload Video + GPS
                                </button>
                    ...
                    {/* Active job progress (shown while a pipeline is running) */}
                    {activeJobId && (
                        <section style={{ marginBottom: '28px' }}>
                            <ProcessingStatus jobId={activeJobId} jobName={activeJobName} />
                        </section>
                    )}
                    ...
                    {/* Jobs Table — real data from /api/v1/desktop/jobs */}
                    <section className="dh-jobs-card">
                        ...
                        <div className="dh-table-wrap">
                            <table className="dh-table">
                                ...
                                <tbody>
                                    {jobsLoading ? (
                                        <tr><td colSpan="5" className="dh-text-muted-italic" style={{ padding: '20px 24px' }}>Loading jobs…</td></tr>
                                    ) : jobs.length === 0 ? (
                                        <tr><td colSpan="5" className="dh-text-muted-italic" style={{ padding: '20px 24px' }}>
                                            No jobs yet — upload a video to get started.
                                        </td></tr>
                                    ) : jobs.slice(0, 8).map((job) => (
                                        <tr key={job.job_id} className="dh-table-row">
                                            <td>
                                                {job.status === 'done' && <span className="dh-status dh-status-done">{Icon.check} Done</span>}
                                                {job.status === 'processing' && <span className="dh-status dh-status-proc">{Icon.spinner()} {job.progress}%</span>}
                                                {job.status === 'queued' && <span className="dh-status dh-status-proc">🕐 Queued</span>}
                                                {job.status === 'failed' && <span className="dh-status dh-status-fail">{Icon.x} Failed</span>}
                                                {job.status === 'cancelled' && <span className="dh-status" style={{ color: '#64748b' }}>⛔ Cancelled</span>}
                                            </td>
                                            ...
                                            <td>
                                                {job.status === 'done' ? (
                                                    ...
                                                    <span className="dh-text-fail">Failed</span>
                                                ) : <span className="dh-text-muted-italic">—</span>}
                                            </td>
                                            <td className="dh-duration">
                                                {job.iri_segments > 0 ? `${job.iri_segments} segs` : '—'}
                                            </td>
                                            <td style={{ textAlign: 'right' }}>
                                                ...
                                                {job.status === 'failed' && (
                                                    <button className="dh-retry" onClick={() => setShowUpload(true)}>Retry</button>
                                                )}
                                                {(job.status === 'processing' || job.status === 'queued') && (
                                                    <button
                                                        className="dh-cancel-btn"
            ...
            <footer className="dh-status-bar">
                <div className="dh-status-left">
                    <div className="dh-status-item">
                        ...
                        MODEL: <span className="dh-stat-val">YOLOv8n Ready</span>
                    </div>
                    ...
                    <div className="dh-status-item">
                        ...
                        BACKEND: <span className="dh-stat-green">ONLINE</span>
                    </div>
                </div>
